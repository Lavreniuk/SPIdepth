...
class InferenceHelper:
    def __init__(self, dataset='kitti', device='cuda:0'):
        ...
        if dataset == 'nyu':
            ...
            model = UnetAdaptiveBins.build(n_bins=100, min_val=self.min_depth, max_val=self.max_depth)
            # pretrained_path = "./pretrained/AdaBins_kitti.pt"
            pretrained_path = "./saved_models_hk/ViT4DE-5"
        else:
...
if __name__ == '__main__':
    ...
    #img = Image.open("../2011_09_26/2011_09_26_drive_0023_sync/image_02/data/0000000091.png")
    img = Image.open("./test_imgs/con2.png")
    start = time()
    inferHelper = InferenceHelper()
    centers, pred = inferHelper.predict_pil(img)
    print(f"took :{time() - start}s")
    plt.imsave('./val_results/'+str(iter)+'_colored_full.png',pred.squeeze() ,cmap='magma_r')
