...
class MonoDatasetMultiCam(data.Dataset):
    ...
    def __init__(self,
                 ...
                 is_train=False,
                 img_ext='.png'):
        super(MonoDatasetMultiCam, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        for scale in range(self.num_scales):
            # K = self.K.copy()
            intrinsics_key = folder[folder.rfind('/')+1:] # NOTE: ensure that every line in KV_intrinsics.txt is like this: /path/to/folder fx fy cx cy , but not folder/
            K = self.get_intrinsics(intrinsics_key).copy() # .copy() to avoid multiply overflow
            ...
            inputs[("K3x3", scale)] = torch.from_numpy(K3x3)
